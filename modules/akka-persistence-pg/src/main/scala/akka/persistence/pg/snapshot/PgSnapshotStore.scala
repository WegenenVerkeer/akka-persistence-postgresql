    ...
    database.run {
      ...
      _ flatMap  { entry: SnapshotEntry =>
        ...
        ((entry.payload, entry.json, entry.manifest) match {
           case (Some(payload), _, _)            => deserialize(persistenceId, serialization.deserialize(payload, classOf[Snapshot]).get.data)
           case (_, Some(event), Some(manifest)) => deserialize(persistenceId, snapshotEncoder.fromJson((event, pgExtension.getClassFor[Any](manifest))))
           case _ => sys.error(
             s"""both payload and event are null for snapshot table entry
