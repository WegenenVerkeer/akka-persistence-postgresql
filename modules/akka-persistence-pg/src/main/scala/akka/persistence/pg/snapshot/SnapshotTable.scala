...
case class SnapshotEntry(persistenceId: String,
                         sequenceNr: Long,
                         timestamp: Long,
...
trait SnapshotTable {
  ...
  class SnapshotTable(tag: Tag) extends Table[SnapshotEntry](tag,
    ...
    def * = (persistenceId, sequenceNr, timestamp, manifest, snapshotBinary, snapshotJson) <> (SnapshotEntry.tupled, SnapshotEntry.unapply)
  }
