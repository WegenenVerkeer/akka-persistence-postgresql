...
class PgAsyncSnapshotStore extends akka.persistence.snapshot.SnapshotStore
  ...
  override def saveAsync(metadata: SnapshotMetadata, snapshot: Any): Future[Unit] = {
    ...
    val snapshotEntry = SnapshotEntry(metadata.persistenceId, metadata.sequenceNr, metadata.timestamp, Some(snapshot.getClass.getName), payloadAsBytes, payloadAsJson)
  ...
  private[this] def serializeSnapshot(snapshot: Any): (Option[JsonString], Option[Array[Byte]]) = {
    if (snapshotEncoder.toJson.isDefinedAt(snapshot)) {
      ...
      require(snapshotEncoder.fromJson.isDefinedAt((json, snapshot.getClass)),
